# Use Cases and Actors - MOPA — Medical Oncology Prior Authorization v0.1.1-snapshot-080926
...
* **`order-select` (informational)** — fires when the provider selects a regimen from the order-set, before signing. The CRD service evaluates approvability and returns informational cards. This is advisory — the order has not been committed.
* **`order-sign` (final determination)** — fires when the provider signs the order. The CRD service returns the final binding coverage determination.
...
1. EHR fires standard CDS Hooks`order-select`(informational):
...
1. Provider reviews approvability cards and decides whether to proceed
1. EHR fires standard CDS Hooks`order-sign`(final determination):
...
* A canonical regimen definition (`AntiCancerRegimenPlanDefinition`) SHOULD be available for the ordered regimen
