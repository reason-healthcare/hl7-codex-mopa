...
const PATIENTS = [
  { id: process.env.JANE_SMITH_PATIENT_ID ?? "jane-smith", name: "Jane Smith",   dob: "1972-04-15", mrn: "MRN-001" },
...
function stripXhtml(div: string): string {
  return div.replace(/<[^>]+>/g, " ").replace(/\s{2,}/g, " ").trim();
}
...
export default async function Home() {
  ...
  const narratives = await Promise.all(
    PATIENTS.map((p) => fetchNarrative(fhirBase, p.id)),
  );
  ...
  return (
    <div className="min-h-screen bg-slate-100">
      <nav className="bg-slate-800 text-slate-100 px-6 py-4 flex items-center justify-between">
        ...
        <a href="http://localhost:4000" className="text-xs text-slate-400 hover:text-slate-200 transition-colors">
          ← Hub
        </a>
      </nav>

      <main className="max-w-3xl mx-auto px-6 py-12">
        <h1 className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">Patient List</h1>
                ...
                <>
                  <tr key={p.id} className="border-t border-slate-200 hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3">
                      <Link href={`/patients/${p.id}`} className="text-blue-600 hover:text-blue-700 font-medium">
                        Open Chart →
                      </Link>
                    </td>
                  </tr>
                  {narratives[i] && (
                    <tr key={`${p.id}-narrative`} className="border-t border-slate-100 bg-slate-50/60">
                      <td colSpan={4} className="px-4 pb-3 pt-1 text-xs text-slate-500 leading-relaxed">
                        {narratives[i]}
                      </td>
                    </tr>
                  )}
